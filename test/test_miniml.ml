(* v0.1 test suite: lexer + parser + pretty-printer.

   Strategy:
   - golden tests: pretty-printing a parsed expression yields an exact string,
     pinning down operator precedence and associativity;
   - idempotence: [pp (pp s) = pp s] — parsing then printing is stable;
   - parses-ok: larger programs (comments, ADTs, recursion) parse without error.

   Expressions are parsed via the [expr_main] start symbol; whole programs
   (sequences of [type]/[let] declarations) via [program]. *)
...
(* -- golden: precedence & associativity ----------------------------- *)
...
(* -- idempotence (programs / declarations) -------------------------- *)
