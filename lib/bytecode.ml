(** The stack-VM instruction set.
    ...
    environment (a [Value.t list]); variables are resolved to de Bruijn indices
    at compile time and read with [ACCESS]. Closures capture the environment.
    Function application saves a return frame ([APPLY]) or reuses the current one
    in tail position ([TAILAPPLY]), giving proper tail calls — a tail-recursive
    loop runs in constant frame-stack space.

    Jumps and closure entry points use integer {e labels} during compilation; a
    two-pass assembler ([Compile.assemble]) resolves labels to addresses and
    drops the [LABEL] markers. See [docs/05-bytecode-vm.md]. *)
